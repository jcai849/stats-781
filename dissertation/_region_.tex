\message{ !name(jason-cairns-dissertation.tex)}\documentclass[11pt, a4paper, titlepage]{report}
...
\message{ !name(jason-cairns-dissertation.tex) !offset(-3) }
...
stopwords ``removed'' by replacing them with \mintinline{r}{NA}
...
the function \mintinline{r}{import_excel()} (\cref{lst:import-excel})
being a wrapper around \mintinline{r}{readr::read_excel()}.
...
(\cref{lst:stopwords}) compose stopwords: \mintinline{r}{get_sw()},
which gathers user input, queries the selected lexicon and combines
the two, and \mintinline{r}{determine_stopwords()} which adds a
boolean \mintinline{r}{TRUE | FALSE} column to the input dataframe.
...
named \mintinline{r}{format_data()} (\cref{lst:format-data}) is
...
to them. A closure, \mintinline{r}{get_search()} (\cref{lst:section})
...
created from that closure, such as \mintinline{r}{get_cantos()},
\mintinline{r}{get_sections()}, etc. The sections are added to the
dataframe with \mintinline{r}{section()}. These have been given as a
...
\printbibliography{}
\end{document}
\message{ !name(jason-cairns-dissertation.tex) !offset(-1663) }
