...
both developed lexicons as well as user input. Two functions compose stopwords in the programl \mintinline{r}{get_sw()}, which gathers user input, queries the selected lexicon, and combines the two, and \mintinline{r}{determine_stopwords()}, which adds a boolean \mintinline{r}{TRUE | FALSE} column to the input dataframe. The following code fragment defines \mintinline{r}{get_sw()}:
\begin{minted}[frame=lines,fontsize=\scriptsize,xleftmargin=\parindent,linenos]{r}
...
With \mintinline{r}{determine_stopwords()} given by:
\begin{minted}[frame=lines,fontsize=\scriptsize,xleftmargin=\parindent,linenos]{r}
