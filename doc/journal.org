...
column", which the insight is performed upon.
...
=get_bigram= requires a means of attaining a bigram for each element
in a vector, skipping NA values. The following function achieves that:
#+begin_src R :session rsession1 :tangle ~/stats-781/src/word-insight.R
...
Working as follows; if our list appears as (1 2 X 4 5 X 7 X), we
