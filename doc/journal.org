[2019-07-14 Sun]: GOTO [[Program Dependencies]], continue through Preparation, Insight, and Visualisation
* COLOPHON
...
*** TODO Get Document Info [0/3]
These are some helper functions to attain the document name and type

TODO:
- [ ] get doc name (for ID)
- [ ] stamp doc_id
- [ ] get doc type (for wrapper)
*** Import .txt
...
Gutenberg texts
*** TODO Import
The wrapper function takes in the filename, and other relevent
...
#+end_src
** TODO [#A] Wrapper
The =preparation= wrapper takes all combinations of features and
intelligently connects them for the "insight column", which the
insight is performed upon. Right now, the only user options are which
file is to be imported, stopword options, and what groups are to be
selected. Lemmatisation is automatic, but in the future this will be a
user option, including whether to perform stopword selection on
lemmatised forms or not, and whether or not to use stemming. This is
because the control structure is complex and program correctness is
...
TODO:
- [ ] Have all insight functions work on vector input and output, so
...
explained previously. The =textrank= package is used to perform textrank.
#+begin_src R :session rsession1 :tangle ~/stats-781/src/word-insight.R
...
however the wrapper may have to be different. This is TBC
*** Word Count
