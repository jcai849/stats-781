...
values following NA will repeat. I will fix this soon enough.
#+begin_src R :session rsession1 :tangle ~/inzightta/R/insight-term.R :eval never-export :exports both
...
#+begin_src R :session rsession1 :tangle ~/inzightta/R/visualisation.R :eval never-export :exports both
  ...
  #'     colour plot
  #'
  #' @param paging step through page by page (not yet implemented)
  #'
  ...
  #'
  #' @export
  struct_ggpage_ungrouped <- function(.data, col_name, paging, palette){
      q_col_name <- dplyr::enquo(col_name)
      .data %>%
          dplyr::pull(word) %>%
          ggpage::ggpage_build() %>%
...
#+begin_src R :session rsession1 :tangle ~/stats-781/test/tests.R :eval never-export :exports both
  ...
  get_vis(insighted, "Page View", "Term Frequency", facet_by = "", scale_fixed = TRUE, paging = "sfj", palette = "dosfj")
  ...
  server <- function(input, output) {
      ...
      output$vis_options <- renderUI({
          switch(input$vis_type,
                 "Page View" = tagList(selectInput("paging", "Select which page you want to view",
                                                   list(1,2,3,4)),
      ...
      visualisation <- reactive({switch(input$vis_type,
                                        "Page View" = get_vis(insighted(), input$vis_type, input$what_vis, input$vis_facet, input$scale_fixed, input$paging, input$palette),
