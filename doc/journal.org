...
#+begin_src R :session rsession1 :tangle ~/stats-781/src/word-insight.R :eval never-export :exports both
  ## ------------------------------ DEPRECATED ------------------------------
...
#+begin_src R :session rsession1 :tangle ~/stats-781/src/vis-insight.R :eval never-export :exports both
...
#+end_src
* Shiny Frontend
For the user interface, the obvious choice is shiny. We will use the
following as our shiny app:
#+begin_src R :session rsession1 :tangle ~/inzightta-shiny/app.R :eval never-export :exports both
  ...
  server <- function(input, output) {
      ...
      prepped_table <- reactive({
          if (isTruthy(input$filter_pred)){
          filter_table() %>%
      ...
      output$table <- renderTable({
          if (isTruthy(input$lemmatise)){
              prepped_table()
          } else if (isTruthy(input$filter_pred)){
              filter_table()
          } else if (isTruthy(input$file1$datapath)){
              import_table()
          }
      })
      output$vars_to_filter <- renderUI(selectInput("filter_var",
                                                    "select which columns to apply filtering to",
                                                    names(import_table())))
  }
